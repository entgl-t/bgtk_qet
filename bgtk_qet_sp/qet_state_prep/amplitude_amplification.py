...
from utils import  bin_to_num, get_gaussian_phases, get_trace_distance,get_sinh_phases\
...
class QSVTAmplitudeAmplification:
    def __init__(self, func_type, phases,poly_deg,num_qubits=2, binary_num='00',signed=True,parity = 'even',f_parity='even', plus_states=False):
        ...
        #self.func_block_encoding = FuncBlockEncodingQSVT(self.func_type,self.phases, self.poly_deg, self.num_qubits, self.binary_num, self.signed,self.parity,self.plus_states).func_block_encoding_reverse_ord()
    ...
    def get_success_ampl(self):
        ...
        #N_tilde_f = np.sqrt(np.sum([expected_polynom(x) ** 2 for x in x_vals]))
    ...
    def simulate(self, num_shots):
        ...
        #Nf = np.sqrt(np.sum([expected_function(self.func_type,key[:-self.num_ancila], self.num_qubits,self.signed) ** 2 for key in result.keys()]))
        for key,val in result.items():
            ...
            x_vals.append(x_bar)
            #expected_out = expected_function(self.func_type,key[:-self.num_ancila], self.num_qubits, signed=self.signed)

            #x = bin_to_num(key[:-self.num_ancila], self.num_qubits, neg=self.signed)

            #x_vals.append(rescaled_x(x,self.num_qubits,self.signed))
